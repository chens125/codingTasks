...
def flight_cost(city):
    flight_price_list = {
        "london": 200,
        "birmingham": 150,
    }  # Pick the right price from the price list
...
city = get_valid_input(
    "Please enter which city you would like to visit:", ["london", "birmingham"]
)
...
print(
    "2.Your hotel cost for {} days at {} is {} pounds.".format(
        num_nights, hotel, hotel_cost(num_nights, hotel)
    )
)
print(
    "3.Your rental cost for {} days ({}) is {} pounds.".format(
        rental_days, car, car_cost(rental_days, car)
    )
)
total_cost = (
    flight_cost(city) + hotel_cost(num_nights, hotel) + car_cost(rental_days, car)
)
